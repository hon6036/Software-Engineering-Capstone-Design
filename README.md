...
졸음운전 감지 하는 Use Case로 Driver는 Camera를 통해 화면에 얼굴이 찍한다.(Shoot Face use case)  
Camera는 찍힌 얼굴을 캡쳐한다.(Capture Face use case)  
Buzzer는 사용자가 졸음운전이라 인식하면 Driver에 경고음을 발생시킨다.(Warn Sleep use case)  
